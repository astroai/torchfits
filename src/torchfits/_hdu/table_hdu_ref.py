...
class TableHDURef:
    ...
    def _repr_html_(self) -> str:
        ...
        cols_val = str(len(self.columns))
        if self._columns is not None:
            cols_val += " (projected)"
        cols = pyhtml.escape(cols_val)

        html_parts = [
            '<div tabindex="0" aria-label="FITS TableHDURef" style=\'max-height: 400px; overflow: auto; border: 1px solid rgba(128, 128, 128, 0.3); margin-bottom: 1em;\'>',
