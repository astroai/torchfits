...
class TableHDURef:
    ...
    def _repr_html_(self):
        ...
        html_parts = [
            '<div tabindex="0" aria-label="FITS Table HDU Reference" style=\'max-height: 400px; overflow: auto; border: 1px solid rgba(128, 128, 128, 0.3); margin-bottom: 1em;\'>',
        ...
        html_parts.append(
            f"<th colspan='2' style='text-align: left; padding: 8px; position: sticky; top: 0; background-color: var(--theme-ui-colors-background, white); border-bottom: 2px solid rgba(128, 128, 128, 0.3); z-index: 1;'>TableHDURef: {pyhtml.escape(str(name))}</th>"
        )
        html_parts.append("</tr></thead><tbody>")

        props = [("Rows", self.num_rows)]

        if self._columns is not None:
            props.append(("Columns (Projected)", len(self.columns)))
        else:
            props.append(("Columns", len(self.columns)))
